class Solution {
    public int leastInterval(char[] tasks, int n) {
        ...
        while (!taskNumMaxHeap.isEmpty() || !waitingQueue.isEmpty()) {
            ...
            if (fromMaxHeap != null && fromMaxHeap[1] <= currCycle) {
                if(fromQueue != null && fromQueue[1] <= currCycle && fromQueue[0] > fromMaxHeap[0]) {
            ...
            } else if (fromQueue != null && fromQueue[1] <= currCycle) {
                if(fromMaxHeap != null && fromMaxHeap[1] <= currCycle && fromMaxHeap[0] > fromQueue[0]) {
                    taskNumMaxHeap.poll();
                    if (fromMaxHeap[0] > 1) {
                        waitingQueue.add(new int[]{fromMaxHeap[0] - 1, currCycle + n + 1});
                    }
